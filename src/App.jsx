...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50/10 via-stone-50/50 to-amber-100/30 text-stone-900 font-light overflow-hidden">
        {isAdminMode && (
            <div className="fixed top-0 left-0 right-0 z-[60] bg-yellow-400 text-black px-6 py-2 flex items-center justify-between shadow-lg">
        ...
        <header className={`fixed ${isAdminMode ? 'top-11' : 'top-0'} left-0 right-0 z-40 transition-all duration-300 ${scrollY > 50 ? 'bg-white/80 backdrop-blur-md border-b border-stone-200/50 shadow-sm' : 'bg-transparent'}`}>
            <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                ...
                <nav className="hidden lg:flex items-center gap-12">{categories.map(category => (<button key={category.id} onClick={() => handleCategoryChange(category.id)} className={`text-sm tracking-wide transition-all duration-500 relative group ${selectedCategory === category.id ? 'text-stone-800 font-medium' : 'text-stone-400 hover:text-stone-700'}`}>{category.name}<div className={`absolute -bottom-2 left-0 h-px bg-gradient-to-r from-amber-400 to-stone-600 transition-all duration-500 ${selectedCategory === category.id ? 'w-full' : 'w-0 group-hover:w-full'}`}></div></button>))}</nav>
        ...
        <main>
            ...
            <section className="py-12 border-y border-stone-200/50 bg-white/60 backdrop-blur-sm sticky top-[72px] z-30">
                <div className="max-w-7xl mx-auto px-6 flex items-center justify-between"><div className="text-sm tracking-wide text-stone-600">{filteredProducts.length} פריטים</div><div className="flex items-center gap-2"><button onClick={exportImagesList} className="flex items-center gap-2 text-sm hover:text-amber-700 p-2 rounded-md"><Download className="w-4 h-4" /> ייצא רשימת מוצרים</button></div></div>
            </section>
            <section className="py-20 bg-gradient-to-b from-transparent via-stone-50/20 to-transparent">
                <div className="max-w-7xl mx-auto px-6"><div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>{filteredProducts.map((product, index) => (<div key={product.id} className="group cursor-pointer animate-fadeInUp relative" onClick={() => handleProductSelect(product)} onMouseEnter={() => setHoveredProduct(product.id)} onMouseLeave={() => setHoveredProduct(null)} style={{animationDelay: `${index * 100}ms`}}>{isAdminMode && (<div className="absolute top-4 left-4 z-20 flex gap-2"><button onClick={(e) => {e.stopPropagation(); setEditingProduct(product);}} className="bg-blue-500 text-white p-2 rounded-full shadow-lg hover:bg-blue-600"><Edit3 className="w-4 h-4" /></button><button onClick={(e) => {e.stopPropagation(); deleteProduct(product.id);}} className="bg-red-500 text-white p-2 rounded-full shadow-lg hover:bg-red-600"><Trash2 className="w-4 h-4" /></button></div>)}<div className="relative overflow-hidden mb-6 bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:scale-105">{product.isNew && <div className="absolute top-4 right-4 z-10 text-xs bg-stone-800 text-white px-3 py-1 rounded-full">חדש</div>}{product.images && product.images.length > 0 && (<img src={hoveredProduct === product.id ? (product.images[1] || product.images[0]) : product.images[0]} alt={product.name} className="w-full h-80 lg:h-96 object-cover" />)}</div><div className="text-center"><div className="text-xs tracking-widest text-amber-700 font-medium">{product.brand}</div><h3 className="text-xl font-medium mt-2">{product.name}</h3><div className="text-lg font-semibold mt-1">₪{product.price}</div></div></div>))}</div></div>
            </section>
        </main>
