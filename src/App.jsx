...
const AddCategoryModal = ({ onSave, onCancel }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (name.trim()) {
            onSave({ id: name.trim().toLowerCase().replace(/\s+/g, '-'), name: name.trim() });
        }
    };
    return (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-2xl flex items-center justify-center p-6">
...
const App = () => {
  ...
  const handleShare = async (product) => { const shareData = { title: product.name, text: `תראו מה מצאתי ב${storeInfo.name}: ${product.name}`, url: window.location.href }; if (navigator.share) { try { await navigator.share(shareData); } catch (err) { console.error("Error sharing:", err); } } else { navigator.clipboard.writeText(shareData.url); alert("הקישור הועתק!"); } };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-amber-50/10 via-stone-50/50 to-amber-100/30 text-stone-900 font-light overflow-hidden">
        ...
        <main>
            ...
            <section className="py-16"><div className="max-w-7xl mx-auto px-6"><div className="aspect-w-16 aspect-h-9"><iframe src="https://embed.screenapp.io/app/#/shared/Xn9POKRa2L?embed=true" title="Product Video" className="w-full h-full rounded-2xl shadow-xl min-h-[500px]" frameBorder="0" allowFullScreen></iframe></div></div></section>
            ...
            <section className="py-20 bg-gradient-to-b from-transparent via-stone-50/20 to-transparent"><div className="max-w-7xl mx-auto px-6"><div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>{filteredProducts.map((product, index) => (<div key={product.id} className="group animate-fadeInUp relative" style={{animationDelay: `${index * 100}ms`}}><div className="cursor-pointer" onClick={() => handleProductSelect(product)}><div className="relative overflow-hidden mb-6 bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:scale-105" onMouseEnter={() => setHoveredProduct(product.id)} onMouseLeave={() => setHoveredProduct(null)}>{product.isNew && <div className="absolute top-4 right-4 z-10 text-xs bg-stone-800 text-white px-3 py-1 rounded-full">חדש</div>}{product.images && product.images.length > 0 && (<img src={hoveredProduct === product.id && product.images.length > 1 ? product.images[1] : product.images[0]} alt={product.name} className="w-full h-80 lg:h-96 object-cover" />)}</div></div><div className="flex justify-between items-center"><div className="text-center flex-grow"><div className="text-xs tracking-widest text-amber-700 font-medium">{product.brand}</div><h3 className="text-xl font-medium mt-2">{product.name}</h3><div className="text-lg font-semibold mt-1">₪{product.price}</div></div><button onClick={(e) => {e.stopPropagation(); handleShare(product);}} title="שתף" className="text-stone-400 hover:text-stone-800 self-start p-2"><Share2 className="w-5 h-5" /></button></div>{isAdminMode && (<div className="absolute top-4 left-4 z-20 flex gap-2"><button onClick={(e) => {e.stopPropagation(); setEditingProduct(product);}} className="bg-blue-500 text-white p-2 rounded-full shadow-lg hover:bg-blue-600"><Edit3 className="w-4 h-4" /></button><button onClick={(e) => {e.stopPropagation(); deleteProduct(product.id);}} className="bg-red-500 text-white p-2 rounded-full shadow-lg hover:bg-red-600"><Trash2 className="w-4 h-4" /></button></div>)}</div>))}</div></div></section>
        </main>
